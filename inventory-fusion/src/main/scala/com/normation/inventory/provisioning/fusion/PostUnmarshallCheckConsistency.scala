 ...
 /* private[this] def checkRoot(report:InventoryReport): Box[InventoryReport] = {
  ...
  private[this] def checkPolicyServer(report:InventoryReport): Box[InventoryReport] = {
    ...
    for {
      ...
      idHere <- if(report.node.main.agents.first.policyServerUUID.get.value == tagHere) Full("OK")
  ...
  private[this] def checkAgent(report:InventoryReport): Box[InventoryReport] = {
    ...
    for {
      ...
      idHere <- Box(Some(report.node.main.agents.first.name)) ?~! "Agent name is not correctly set (but tag '%s' is present with value '%s'".format(tag, tagHere)
    } yield {
      report
    }
  }
